...
def init_known_flags(s, bm):
    for function in bm.functions:
        x_known = []
        for i in range(function.tablesize()):
            x_known.append(s.add_lit())
        bm.add_known_flags(function.name, x_known)
...
def get_a_b(function, a_name, b):
    ...
    if "+" in function and a_name.startswith("!"):
        a_name = a_name[1:]
        return (a_name, b)
...
def add_clauses_3(s, bm, b_map):
    for function in bm.functions:
        ...
        experimental_row_ands = function.experimental_row_ands
        b = b_map[len(function.inputs)]
        y = function.y
        x = function.x
        for e in range(bm.nr_of_experiments):
            ...
            for row in range(size):
                ...
                s.add_clause(inis)
                rows.append(inputs)
            experimental_row_ands.append(row_ands)
...
# Set flags for values that are truly set to be able to filter out these models
# Not needed for already known functions (aka name contains +)
def set_known_values(s, bm, b_map):
    for function in bm.functions:
        if "+" not in function.name:
            ...
            experimental_row_ands = function.experimental_row_ands
            for row in range(size):
                not_f = [-function.x_flags[row]]
                for experiment in range(bm.nr_of_experiments):
                    exp = selector[experiment]
                    clause = [function.x_flags[row]]
                    #not_f = [-function.x_flags[row]]
                    for input in range(len(function.inputs)):
                        #s.add_clause([-function.x_flags[row], exp[row][input]])
                        clause.append(-exp[row][input])
                    s.add_clause(clause)
                    not_f.append(experimental_row_ands[experiment][row])
                    #s.add_clause(not_f)
                s.add_clause(not_f)
